...
class MapPageController extends GetxController {
  ...
  Future<void> fetchWeather() async {
    ...
    final url =
        "https://api.openweathermap.org/data/2.5/weather?q=Sfax&appid=$apiKey&units=metric&lang=$lang";
    try {
        ...
        .listen((snap) {
          ...
          myMarkers.value = snap.docs.map((doc) {
            ...
            if (selectedLayer.value == 'azote') {
              val = (data['azote'] ?? 0.0).toDouble();
            } else if (selectedLayer.value == 'maladies') {
              val = (data['sante'] ?? 0.0).toDouble();
            } else if (selectedLayer.value == 'ph') {
              val = (data['ph'] ?? 0.0).toDouble();
            } else {
              val = (data['humidity'] ?? 0.0).toDouble();
            }
  ...
  Widget _buildMarker(double valeur, String type) {
    ...
    if (type == 'stress_hydrique' && valeur < 30) {
      color = Colors.red;
    } else if (type == 'azote' && valeur < 40) {
      color = Colors.orange;
    } else if (type == 'maladies' && valeur < 60) {
      color = Colors.purple;
    } else if (type == 'ph' && (valeur < 6.0 || valeur > 8.0)) {
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "${selectedLayer.value.tr}: ${val.toStringAsFixed(1)}",
